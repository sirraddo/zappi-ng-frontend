    ...
    <div style={{display:"flex",gap:16,justifyContent:"center",margin:"28px 0 8px"}}>
      {Array.from({length}).map((_,i)=>(
        <div key={i} style={{
          ...
          border:`2.5px solid ${error?C.danger:i<value.length?C.primary:"#CCC"}`,
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,maxWidth:280,margin:"16px auto 0"}}>
      {keys.map((k,i)=>(
        <button key={i} onClick={()=>k==="⌫"?onDelete():k?onPress(k):null} disabled={!k}
          style={{height:62,borderRadius:14,border:"none",
            background:k==="⌫"?"#FEE2E2":k?"white":"transparent",
...
export function RegisterScreen({ onSuccess, onLogin }) {
  const [step,setStep]=useState("form") // form | verify | pin | txnpin
  ...
  const field=(key,label,placeholder,type="text")=>(
    <div style={{marginBottom:14}}>
      <p style={{fontSize:12,fontWeight:700,color:"#555",margin:"0 0 6px",textTransform:"uppercase",letterSpacing:"0.5px"}}>{label}</p>
      <div style={{position:"relative"}}>
        <input value={form[key]} onChange={e=>setForm({...form,[key]:e.target.value})}
          type={type==="password"?(showPass?"text":"password"):type}
          placeholder={placeholder}
          style={{width:"100%",padding:"13px 14px",borderRadius:12,border:`1.5px solid ${errors[key]?"#EF4444":"#E5E7EB"}`,boxSizing:"border-box",fontSize:14,outline:"none",fontFamily:"inherit",background:"white"}}/>
  ...
  if(step==="pin") return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <p style={{margin:0,fontSize:13,color:"#777",textAlign:"center"}}>4-digit PIN to unlock Zappi NG</p>
  ...
  if(step==="txnpin") return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <p style={{margin:0,fontSize:13,color:"#777",textAlign:"center",maxWidth:260}}>6-digit PIN required before every payment</p>
      ...
      <button onClick={onSuccess} style={{background:"none",border:"none",color:"#aaa",fontSize:13,cursor:"pointer",marginTop:16,padding:8}}>Skip for now</button>
    </div>
  )
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,overflowY:"auto"}}>
      ...
      <div style={{padding:20}}>
        <p style={{fontSize:13,fontWeight:700,color:"#555",margin:"0 0 10px"}}>Choose your avatar</p>
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:20}}>
          {avatars.map(a=>(
            <button key={a} onClick={()=>setForm({...form,avatar:a})}
              style={{width:44,height:44,borderRadius:12,border:`2px solid ${form.avatar===a?C.primary:"#E5E7EB"}`,background:form.avatar===a?C.light:"white",fontSize:22,cursor:"pointer",transition:"all 0.2s"}}>{a}</button>
          ))}
        </div>
        ...
        <p style={{textAlign:"center",fontSize:11,color:"#bbb",marginTop:8}}>By creating an account you agree to our Terms of Service and Privacy Policy</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,padding:20}}>
        <div style={{display:"flex",gap:8,background:"white",borderRadius:14,padding:4,marginBottom:24,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
          {[{id:"password",label:"🔒 Password"},{id:"pin",label:"🔢 PIN"}].map(m=>(
            <button key={m.id} onClick={()=>{setMethod(m.id);setError("");setPinError("");setPin("")}}
              style={{flex:1,padding:"10px 8px",borderRadius:10,border:"none",background:method===m.id?C.primary:"transparent",color:method===m.id?"white":"#777",fontWeight:700,fontSize:13,cursor:"pointer",transition:"all 0.2s"}}>{m.label}</button>
          ))}
        </div>

        {method==="password"&&(
          <div>
            <p style={{fontSize:12,fontWeight:700,color:"#555",margin:"0 0 6px",textTransform:"uppercase",letterSpacing:"0.5px"}}>Email or Phone</p>
            ...
            <p style={{fontSize:12,fontWeight:700,color:"#555",margin:"0 0 6px",textTransform:"uppercase",letterSpacing:"0.5px"}}>Password</p>
            ...
            <div style={{textAlign:"center",margin:"20px 0",display:"flex",alignItems:"center",gap:12}}>
              ...
              <span style={{fontSize:12,color:"#aaa",fontWeight:500}}>or sign in with</span>
              <div style={{flex:1,height:1,background:"#E5E7EB"}}/>
            </div>
            <button style={{width:"100%",background:"white",border:"1.5px solid #E5E7EB",borderRadius:14,padding:14,fontSize:14,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}
        ...
        {method==="pin"&&(
          <div style={{textAlign:"center"}}>
            <p style={{fontSize:14,color:"#555",margin:"0 0 4px"}}>Enter your 4-digit login PIN</p>
...
export function ForgotScreen({ onBack }) {
  ...
  if(step==="success") return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}>
      ...
      <p style={{fontSize:14,color:"#777",maxWidth:280,lineHeight:1.6,margin:"0 0 32px"}}>We've sent a password reset link to <strong>{email}</strong>. Check your inbox and follow the instructions.</p>
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <p style={{fontSize:14,color:"#777",textAlign:"center",margin:"0 0 28px",maxWidth:280}}>Enter your email or phone number and we'll send you a reset link.</p>
      <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email or phone number"
        style={{width:"100%",padding:14,borderRadius:14,border:`1.5px solid ${error?"#EF4444":"#E5E7EB"}`,boxSizing:"border-box",fontSize:14,outline:"none",marginBottom:8,fontFamily:"inherit"}}/>
...
export function TxnPinModal({ onSuccess, onCancel, label="Confirm Payment" }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",zIndex:9999,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div style={{width:"100%",maxWidth:430,background:"white",borderRadius:"24px 24px 0 0",padding:"24px 20px 48px"}}>
        ...
        <p style={{margin:"0 0 4px",fontSize:13,color:"#999"}}>Enter your 6-digit transaction PIN to proceed</p>
...
export function ProfileScreen({ onBack, onLogout }) {
  ...
  const [section,setSection]=useState(null) // changePin | changeTxnPin | changePassword
  const [pinState,setPinState]=useState({step:"current",current:"",newPin:"",confirm:"",error:""})
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg}}>
      ...
      <div style={{display:"flex",gap:0,margin:"0 16px",marginTop:-20,background:"white",borderRadius:16,overflow:"hidden",boxShadow:"0 4px 16px rgba(0,0,0,0.08)"}}>
        {stats.map((s,i)=>(
          <div key={s.label} style={{flex:1,padding:"16px 8px",textAlign:"center",borderRight:i<2?"1px solid #F3F4F6":"none"}}>
            ...
            <p style={{margin:"4px 0 0",fontSize:11,color:"#999"}}>{s.label}</p>
          </div>
        ))}
      </div>
        ...
        <div style={{background:"white",borderRadius:16,padding:16,marginBottom:12,boxShadow:"0 2px 8px rgba(0,0,0,0.05)"}}>
          ...
          {editing&&(
            <div style={{marginBottom:12}}>
              <p style={{fontSize:12,fontWeight:700,color:"#555",margin:"0 0 6px"}}>Avatar</p>
              <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
                {avatars.map(a=>(
                  <button key={a} onClick={()=>setForm({...form,avatar:a})} style={{width:38,height:38,borderRadius:10,border:`2px solid ${form.avatar===a?C.primary:"#E5E7EB"}`,background:form.avatar===a?C.light:"white",fontSize:18,cursor:"pointer"}}>{a}</button>
                ))}
              </div>
            </div>
          )}
          {[{key:"fullName",label:"Full Name"},{key:"email",label:"Email"},{key:"phone",label:"Phone"}].map(f=>(
            <div key={f.key} style={{marginBottom:10}}>
              <p style={{fontSize:11,color:"#aaa",margin:"0 0 4px",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.5px"}}>{f.label}</p>
              {editing
                ...
                :<p style={{margin:0,fontSize:14,fontWeight:500,color:"#1a1a1a"}}>{user[f.key]||"—"}</p>
              }
            </div>
          ))}
        </div>

        <div style={{background:"white",borderRadius:16,padding:4,marginBottom:12,boxShadow:"0 2px 8px rgba(0,0,0,0.05)"}}>
          <p style={{fontSize:12,fontWeight:700,color:"#aaa",margin:"12px 14px 8px",textTransform:"uppercase",letterSpacing:"0.5px"}}>Security</p>
          {[
            ...
            {icon:"👆",label:"Fingerprint Login",sub:"Enable biometric authentication",action:()=>alert("Fingerprint login is available on mobile devices")},
          ].map(item=>(
            <button key={item.label} onClick={item.action} style={{width:"100%",background:"none",border:"none",padding:"12px 14px",display:"flex",alignItems:"center",gap:12,cursor:"pointer",borderRadius:12,textAlign:"left"}}>
              ...
              <div style={{flex:1}}>
                <p style={{margin:0,fontSize:14,fontWeight:600,color:"#1a1a1a"}}>{item.label}</p>
                <p style={{margin:"2px 0 0",fontSize:12,color:"#aaa"}}>{item.sub}</p>
              </div>
